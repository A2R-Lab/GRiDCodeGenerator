def gen_aba_inner_floating(self, use_thread_group = False):
    ...
    self.gen_add_code_line("// Diagnostic fallback: reuse the validated floating FD composition.")
    ...
    for jid in range(NJ - 1, -1, -1):
        ...
        if jid == 0:
            ...
            self.gen_add_end_control_flow()
            self.gen_add_sync(use_thread_group)
            self.gen_add_code_line("#if GRID_CUDA_DEBUG_FLOATING_ABA")
            self.gen_add_code_line("if(threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x == 0){", True)
            self.gen_add_code_line("printf(\"ABA_DEBUG root_D_pre_invert\"); for (int dbg = 0; dbg < 36; dbg++){ printf(\" %.9g\", static_cast<double>(s_temp[" + str(fbDOffset) + " + dbg])); } printf(\"\\n\");")
            self.gen_add_end_control_flow()
            self.gen_add_code_line("#endif")
            ...
            self.gen_add_code_line("#if GRID_CUDA_DEBUG_FLOATING_ABA")
            self.gen_add_code_line("if(threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x == 0){", True)
            self.gen_add_code_line("printf(\"ABA_DEBUG root_IA\"); for (int dbg = 0; dbg < 36; dbg++){ printf(\" %.9g\", static_cast<double>(s_temp[" + str(IAOffset) + " + dbg])); } printf(\"\\n\");")
            self.gen_add_code_line("printf(\"ABA_DEBUG root_pA\"); for (int dbg = 0; dbg < 6; dbg++){ printf(\" %.9g\", static_cast<double>(s_temp[" + str(pAOffset) + " + dbg])); } printf(\"\\n\");")
            self.gen_add_code_line("printf(\"ABA_DEBUG root_D\"); for (int dbg = 0; dbg < 36; dbg++){ printf(\" %.9g\", static_cast<double>(s_temp[" + str(fbDOffset) + " + dbg])); } printf(\"\\n\");")
            self.gen_add_code_line("printf(\"ABA_DEBUG root_Dinv\"); for (int dbg = 0; dbg < 36; dbg++){ printf(\" %.9g\", static_cast<double>(s_temp[" + str(fbDinvOffset) + " + dbg])); } printf(\"\\n\");")
            self.gen_add_code_line("printf(\"ABA_DEBUG root_rhs_pre_gravity\"); for (int dbg = 0; dbg < 6; dbg++){ printf(\" %.9g\", static_cast<double>(s_temp[" + str(fbRhsOffset) + " + dbg])); } printf(\"\\n\");")
            self.gen_add_end_control_flow()
            self.gen_add_code_line("#endif")
            continue
        ...
        if parent != -1:
            ...
            self.gen_add_parallel_loop("row", "6", use_thread_group)
            self.gen_add_code_line("s_temp[" + str(tempVecOffset) + " + row] = dot_prod<T,6,1,1>(&s_XImats[" + str(36 * jid) + " + 6*row], &s_temp[" + str(UOffset + jid6) + "]);")
            self.gen_add_end_control_flow()
            self.gen_add_sync(use_thread_group)
            self.gen_add_parallel_loop("row", "6", use_thread_group)
            ...
            self.gen_add_parallel_loop("ind", "36", use_thread_group)
            self.gen_add_code_line("int row = ind % 6; int col = ind / 6;")
            self.gen_add_code_line("s_temp[" + str(tempVecOffset) + " + row + 6*col] = dot_prod<T,6,1,1>(&s_XImats[" + str(36 * jid) + " + 6*row], &s_temp[" + str(IAOffset + 36 * jid) + " + 6*col]);")
            self.gen_add_end_control_flow()
            self.gen_add_sync(use_thread_group)
            self.gen_add_parallel_loop("ind", "36", use_thread_group)
            self.gen_add_code_line("int row = ind % 6; int col = ind / 6;")
            self.gen_add_code_line("s_temp[" + str(tempMatOffset) + " + row + 6*col] = dot_prod<T,6,6,1>(&s_temp[" + str(tempVecOffset) + " + row], &s_XImats[" + str(36 * jid) + " + 6*col]);")
            ...
            self.gen_add_sync(use_thread_group)
            self.gen_add_parallel_loop("row", "6", use_thread_group)
            self.gen_add_code_line("s_temp[" + str(tempVecOffset) + " + row] = dot_prod<T,6,1,1>(&s_XImats[" + str(36 * jid) + " + 6*row], &s_temp[" + str(paOffset + jid6) + "]) + s_temp[" + str(UOffset + jid6) + " + row] * s_temp[" + str(uOffset + jid) + "] / s_temp[" + str(dOffset + jid) + "];")
            self.gen_add_end_control_flow()
            self.gen_add_sync(use_thread_group)
    ...
    for bfs_level in range(n_bfs_levels):
        ...
        if bfs_level == 0:
            ...
            self.gen_add_code_line("#if GRID_CUDA_DEBUG_FLOATING_ABA")
            self.gen_add_code_line("if(threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x == 0){", True)
            self.gen_add_code_line("printf(\"ABA_DEBUG root_gravity_a\"); for (int dbg = 0; dbg < 6; dbg++){ printf(\" %.9g\", static_cast<double>(s_va[" + str(6 * NJ) + " + dbg])); } printf(\"\\n\");")
            self.gen_add_code_line("printf(\"ABA_DEBUG root_rhs_post_gravity\"); for (int dbg = 0; dbg < 6; dbg++){ printf(\" %.9g\", static_cast<double>(s_temp[" + str(fbRhsOffset) + " + dbg])); } printf(\"\\n\");")
            self.gen_add_code_line("printf(\"ABA_DEBUG root_qdd\"); for (int dbg = 0; dbg < 6; dbg++){ printf(\" %.9g\", static_cast<double>(s_qdd[dbg])); } printf(\"\\n\");")
            self.gen_add_end_control_flow()
            self.gen_add_code_line("#endif")
            self.gen_add_parallel_loop("row", "6", use_thread_group)
            self.gen_add_code_line("int fb_col = row < 3 ? row + 3 : row - 3;")
        ...
        for jid in inds:
            ...
            if jid == 1:
                self.gen_add_code_line("#if GRID_CUDA_DEBUG_FLOATING_ABA")
                self.gen_add_code_line("printf(\"ABA_DEBUG joint1_U_parent\"); for (int dbg = 0; dbg < 6; dbg++){ printf(\" %.9g\", static_cast<double>(s_temp[" + str(UOffset + jid6) + " + dbg])); } printf(\"\\n\");")
                self.gen_add_code_line("printf(\"ABA_DEBUG joint1_parent_a\"); for (int dbg = 0; dbg < 6; dbg++){ printf(\" %.9g\", static_cast<double>(s_va[" + str(6 * NJ + parent6) + " + dbg])); } printf(\"\\n\");")
                self.gen_add_code_line("printf(\"ABA_DEBUG joint1_d_u_qdd %.9g %.9g %.9g\\n\", static_cast<double>(s_temp[" + str(dOffset + jid) + "]), static_cast<double>(s_temp[" + str(uOffset + jid) + "]), static_cast<double>(s_qdd[" + str(dof) + "]));")
                self.gen_add_code_line("#endif")
            self.gen_add_end_control_flow()
            self.gen_add_sync(use_thread_group)
            self.gen_add_parallel_loop("row", "6", use_thread_group)
            self.gen_add_code_line("s_va[" + str(6 * NJ + jid6) + " + row] = dot_prod<T,6,6,1>(&s_XImats[" + str(36 * jid) + " + row], &s_va[" + str(6 * NJ + parent6) + "]) + s_temp[" + str(cOffset + jid6) + " + row];")
