def gen_aba_inner_floating(self, use_thread_group = False):
    ...
    for bfs_level in range(n_bfs_levels):
        ...
        for jid in inds:
            ...
            self.gen_add_code_line(f"grid_linalg_row_strided_gemv<T,6,6,6>(&s_XImats[{36*jid}], &s_va[{parent6}], &s_va[{jid6}], static_cast<T>(1), static_cast<T>(0), {self.linalg_smem_for(6,6,6)});")
    ...
    for jid in range(NJ - 1, -1, -1):
        ...
        if parent != -1:
            ...
            self.gen_add_code_line(f"grid_linalg_gemm<T,6,6,6>(&s_temp[{tempVecOffset}], &s_XImats[{36*jid}], &s_temp[{tempMatOffset}], static_cast<T>(1), static_cast<T>(0), {self.linalg_smem_for(6,6,6)});")
            self.gen_add_parallel_loop("ind", "36", use_thread_group)
            self.gen_add_code_line("int row = ind % 6; int col = ind / 6;")
    ...
    for bfs_level in range(n_bfs_levels):
        ...
        for jid in inds:
            ...
            self.gen_add_end_control_flow()
            self.gen_add_sync(use_thread_group)
            self.gen_add_code_line(f"grid_linalg_row_strided_gemv<T,6,6,6>(&s_XImats[{36*jid}], &s_va[{6*NJ + parent6}], &s_va[{6*NJ + jid6}], static_cast<T>(1), static_cast<T>(0), {self.linalg_smem_for(6,6,6)});")
            self.gen_add_parallel_loop("row", "6", use_thread_group)
...
def gen_aba_inner(self, use_thread_group = False): 
    ...
    for bfs_level in range(n_bfs_levels):
        ...
        if bfs_level == 0:
            ...
            for jid_val in inds:
                ...
                self.gen_add_code_line(f"grid_linalg_row_strided_gemv<T,6,6,6>(&s_XImats[{36*jid_val}], &s_va[{6*parent_val}], &s_va[{6*jid_val}], static_cast<T>(1), static_cast<T>(0), {self.linalg_smem_for(6,6,6)});")
                self.gen_add_serial_ops(use_thread_group)
        ...
        if bfs_level != 0:
            if len(inds) > 1 and self.robot.has_repeated_parents(inds):
                ...
                for jid_val in inds:
                    ...
                    self.gen_add_code_line(f"grid_linalg_gemm<T,6,6,6>(&s_temp[{98*n + 36*jid_val}], &s_XImats[{36*jid_val}], &s_temp[{36*parent_val}], static_cast<T>(1), static_cast<T>(1), {self.linalg_smem_for(6,6,6)});")
    ...
    for bfs_level in range(n_bfs_levels):
        ...
        # calculate a where parent is not base
            ...
            for jid_val in inds:
                parent_val = self.robot.get_parent_id(jid_val)
                self.gen_add_code_line(f"grid_linalg_row_strided_gemv<T,6,6,6>(&s_XImats[{36*jid_val}], &s_va[{6*n + 6*parent_val}], &s_va[{6*n + 6*jid_val}], static_cast<T>(1), static_cast<T>(0), {self.linalg_smem_for(6,6,6)});")
                self.gen_add_parallel_loop("row", "6", use_thread_group)
